...
if [ "$SKIP_DOWNLOAD" = true ]; then
    ...
    echo -e "${CYAN}      [3/6] Counties (~218 features)...${NC}"
    ...
    echo -e "${CYAN}      [4/6] Metropolitan Counties (6 features - E11)...${NC}"
    if [ -f "$METROCOUNTIES_FILE" ] && [ -s "$METROCOUNTIES_FILE" ]; then
        echo -e "${GRAY}            File exists, skipping${NC}"
    else
        node "$DOWNLOAD_SCRIPT" metrocounties "$METROCOUNTIES_FILE"
    fi
    
    echo -e "${CYAN}      [5/6] Local Authority Districts (~361 features)...${NC}"
    ...
    echo -e "${CYAN}      [6/6] Built-up Areas (~8545 features, may take a while)...${NC}"
...
MISSING_FILES=false
for file in "$COUNTRIES_FILE" "$REGIONS_FILE" "$COUNTIES_FILE" "$METROCOUNTIES_FILE" "$LAD_FILE" "$BUA_FILE" "$LONDON_FILE"; do
    if [ ! -f "$file" ]; then
...
INPUT_FILES="${COUNTRIES_FILE},${REGIONS_FILE},${COUNTIES_FILE},${METROCOUNTIES_FILE},${LAD_FILE},${BUA_FILE}"

# Optional: OSM neighbourhoods (generated by extract-osm-neighbourhoods.sh)
NEIGHBOURHOODS_ARGS=()
if [ -f "$NEIGHBOURHOODS_FILE" ] && [ -s "$NEIGHBOURHOODS_FILE" ]; then
    echo -e "${CYAN}      Including OSM neighbourhoods: $NEIGHBOURHOODS_FILE${NC}"
    NEIGHBOURHOODS_ARGS=(--osm-neighbourhoods "$NEIGHBOURHOODS_FILE")
else
    echo -e "${YELLOW}      No OSM neighbourhoods file found ($NEIGHBOURHOODS_FILE)${NC}"
    echo -e "${YELLOW}      Run ./extract-osm-neighbourhoods.sh first to include the neighbourhood layer${NC}"
fi
